...
mod determ_bench {
    ...
    pub fn naive_thread_function(autom : &Autom, word : String) {
        assert_eq!(naive(&autom, &word), true);
    }
    ...
    pub fn quadratic_performance_test() {
        ...
        // Create output file
        let path = "./tests/bench_results/qpt_2204_glueck_hashmap.csv";
        let mut file = fs::File::create(path).expect("File creation failed");
    ...
    #[test]
    pub fn quadratic_performance_test_naive() {
        // Loop params
        let start = 0;
        let step = 30;
        let tests = 400;
        ...
        // Create output file
        let path = "./tests/bench_results/qpt_2204_naive.csv";
        let mut file = fs::File::create(path).expect("File creation failed");
        ...
        for n in (start..(start + step*tests + 1)).step_by(step) {
            ...
            thread::scope(|s| {
                thread::Builder::new().stack_size(0xFFFF * n)
                .spawn_scoped(s, || naive_thread_function(&autom, test_word))
                .unwrap();
            });
